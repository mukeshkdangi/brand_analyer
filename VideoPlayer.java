package csci576;import java.awt.*;
...
int j=0;
            if (!stop)
            {
                while (j < Math.round(audioPlayer.getFramePosition() / numberOfSamplesPerFrame)) {
                ...
                while (j > Math.round(offset + audioPlayer.getFramePosition() / numberOfSamplesPerFrame)) {
                }

                for (int i = j; i < numberOfFrames; i++) {

                    // Video ahead of audio, wait for audio to catch up
                    while (i > Math.round(offset + audioPlayer.getFramePosition() / numberOfSamplesPerFrame)) {
                    }

                    // Audio ahead of video, roll video forward to catch up
                    while (i < Math.round(audioPlayer.getFramePosition() / numberOfSamplesPerFrame)) {
                        readBytes();
                        component.setImg(bufferedImage);
                        Player.controlOptionPanel.add(component);
                        Player.controlOptionPanel.repaint();
